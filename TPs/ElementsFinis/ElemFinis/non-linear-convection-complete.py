...
def dmu_du(u):
    ...
    #return 1.e-30
    return m* (1/u_diri_non_homog)*(1.+(u/u_diri_non_homog))**(m-1)
...
while (error > eps_du and i < i_max):
    ...
    if advection:
        ...
        a = inner(mu_n * grad(du), grad(v)) * dx + inner(vel, grad(du)) * v * dx + inner(dmu_du_n * du * grad(un),
                                                                                         grad(v)) * dx
...
if advection:
    dF = inner(mu(u_nl) * grad(u), grad(v)) * dx + inner(dmu_du(u_nl) * u * grad(u_nl), grad(v)) * dx + inner(vel, grad(
        u)) * v * dx
else:
