...
class vda_river(object):
    ...
    def __init__(
        self, model_type, param_mesh, param_elements, param_geom, param_bc, param_VDA
    ):
        """
        ...
        if not (
            self.model_type == "nonlinear_stationnary"
            or self.model_type == "linear_stationnary"
        ):
        ...
        self.V = FunctionSpace(
            self.mesh,
            param_elements["type of elements"],
            param_elements["degree of freedom"],
        )
        self.nb_cell = param_mesh["number cells"]
        self.L = param_mesh["length river"]

        self.H_up = param_bc["H bc upstream"]  # Dirichlet BC
        self.H_down = param_bc[
            "H bc downstream"
        ]  # Dirichlet BC (not employed if Neumann)
        ...
        self.b = array_to_FEfct(
            self.V, param_geom["bathymetry array"]
        )  # init the bathy to bathy_1st
        ...
        if not (len(param_VDA) == 0):
            ...
            self.Hobs = array_to_FEfct(
                self.V, param_VDA["obs"]
            )  # create Hobs as a FEniCS function
            self.ind_obs = array_to_FEfct(
                self.V, param_VDA["obs index"]
            )  # ind_obs = 1 if obs available, 0 otherwise. create as array
            self.b_b = array_to_FEfct(
                self.V, param_VDA["background"]
            )  # create b_b as a FEniCS function
    ...
    def VDA(self, b0, b_t):
        ...
        if (
            not (self.nb_cell == np.size(b_t))
            or not (self.nb_cell == np.size(b0))
            or not (self.nb_cell == np.size(Href_np))
            or not (self.nb_cell == np.size(Hobs_np))
        ):
        ...
        def callback_fct(bc):
            ...
            if self.jreg_type == "b_b":
                cost_prov, costobs_prov, costreg_prov = self.Cost(
                    array_to_FEfct(self.V, bc), Hb
                )
            else:
            ...
            if self.jreg_type == "b_b":
                djnorm_prov = self.Gradient_Cost(array_to_FEfct(self.V, bc), Hb, Pb_FE)[
                    1
                ]  # norm of the gradient power 2
            else:
                djnorm_prov = self.Gradient_Cost(self.b, Hb, Pb_FE)[
                    1
                ]  # norm of the gradient power 2
            ...
            if (self.jreg_type == "grad") and (
                self.n_iter == 0
            ):  # @1st iteration if 'grad'
                beta = 0.5  # TO BE ADJUSTED
                self.alpha = (
                    beta * costobs_prov / costreg_prov
                )  # tune the initial value of alpha * TO BE IMPROVED
                print(
                    "\nInitial weight of Jreg setting @ ite =",
                    self.n_iter,
                    ". beta =",
                    beta * 100.0,
                    "%. alpha=",
                ...
                print(" ")
            elif (self.jreg_type == "b_b") and (
                self.n_iter == 1
            ):  # @2nd iteration if 'b_b'
                if costreg_prov > 1.0e-12:
                    beta = 0.0  # TO BE ADJUSTED
                    self.alpha = (
                        beta * costobs_prov / costreg_prov
                    )  # tune the initial value of alpha * TO BE IMPROVED
                    print(
                    ...
                    sys.exit(
                        "callback function: jreg_type='b_b', difficulty to set up the initial value of alpha..."
                    )

            # decreasing regularization coeff.
            if self.n_iter % 10 == 0 and self.n_iter != 0:
                if self.jreg_type == "grad":
                    self.alpha *= 0.9  # TO BE ADJUSTED
                else:
                    self.alpha *= 0.9  # TO BE ADJUSTED

            # dimensions test
            if (
                not (self.nb_cell == np.size(FEfct_to_array(self.mesh, self.b)))
                or not (self.nb_cell == np.size(Hb_np))
            ...
            mse_b = (np.square(bc - b_t)).mean
            mse_H = (np.square(Hb_np - Hobs_np)).mean
        ...
        np.save(
            self.path_out + "fitting-threshold.npy", np.array([cost_1, cost_2, cost_3])
        )
        ...
        print(
            "class_vda / VDA: constraints on the control var are imposed (type any key to resume)"
        )
        constr = self.ControlConstraint(
            b_t, b_1st_np, Hobs_np
        )  # inequality / equality constraints on b
        ...
        ftol = 1e-15
        gtol = 1e-10
        maxiter = 100  # MAY BE ADJUSTED

        print(" ")
        print(
            "minimization method = L-BFGS-B, min evol. of j=",
            ftol,
            ", min gradient value=",
            gtol,
            ", maxiter=",
        ...
        res = spo.minimize(
            self.Cost_np,
            b0,
            jac=self.Gradient_Cost_np,
            method="L-BFGS-B",
            options={"disp": True, "ftol": ftol, "gtol": gtol, "maxiter": maxiter},
    ...
    def Observations(self, b, noise_bool):
        ...
        if noise_bool == 1:
            noise_np = np.random.normal(
                0.0, 0.05, np.size(noise_np)
            )  # (standard deviation in SI units) # MAY BE CHANGED
    ...
    def Direct_Model(self, b):
        ...
        # LHS of the equation
        a = (
            inner(grad(H)[0], grad(0.3 * (self.Href - b) / abs(grad(self.Href)[0]) * phi)[0]) * dx
            + grad(H)[0] * phi * dx
        )  # original model linearised
        ...
        bc0_up = DirichletBC(self.V, Constant(0.0), self.bdry_up)
        bc0_down = DirichletBC(self.V, Constant(0.0), self.bdry_down) 

        # Call the linear algebra solver, including the Dirichlet BC
        # Pas possible de faire un solve car non linéaire
        ...
        i = 0
        error = 1.0  # current iteration


        # Initialization point H0
        a0 = inner(grad(H) , grad(0.3*(self.Href-b)/abs(grad(self.Href)[0]) * phi))*dx  + grad(H)[0]*phi*dx # original linear model
        L0 = grad(b)[0]*phi*dx
        H0 = Function(self.V)
        solve(a0 == L0, H0, bc_diri)
        # print("Href = ", self.Href.vector().get_local())
        # print("b = ", b.vector().get_local())
        ...
        while (error>eps_du and i<i_max): 
          ...
          a = inner(grad(Hn), grad(0.3*dH/abs(grad(self.Href)[0])* phi))*dx \
              + inner(grad(dH), grad(0.3*(Hn-b)/abs(grad(self.Href)[0]) * phi))*dx \
              + grad(dH)[0]*phi*dx
          # RHS of the linearized eqn
          L = grad(b)[0]*phi*dx - inner(grad(Hn), grad(0.3*(Hn-b)/abs(grad(self.Href)[0]) * phi))*dx - grad(Hn)[0]*phi*dx

          # Solve
          solve(a == L, dHn, [bc0_up, bc0_down])
        #   print(norm(Hn, 'l2'))
          error = norm(dHn, 'l2')/ norm(Hn, 'l2')
        ...
        # plt.figure()
        # # plot(mesh)
        # p = plot(Hn, title="The non linear solution (home-made solver)")
        # # p.set_cmap("rainbow")
        # plt.colorbar(p)
        # # plt.show()
        ...
        if not (self.alpha == 0.0):
            if self.jreg_type == "grad":  # regularization term = "grad"
                ...
                sys.exit(
                    "The regularization "
                    + self.jreg_type
                    + " is not implemented, it must be either <b_b> or <grad>"
                )
        else:
            Jreg = 0.0
    ...
    def Adjoint_Model(self, b, Hb):
        ...
        p = TrialFunction(
            self.V
        )  # trial function, the adjoint state, solution of the adjoint eqn
        phi = TestFunction(self.V)  # test function

        # Boundary conditions
        bc_up = DirichletBC(
            self.V, Constant(0.0), self.bdry_up
        )  # upstream boundary condition
        bc_diri = [bc_up]
        if self.bc_type_out == "dirichlet":
            bc_down = DirichletBC(
                self.V, Constant(0.0), self.bdry_down
        ...
        a = (
            inner(
                grad((0.3 * (self.Href - b) / abs(grad(self.Href)[0])) * p), grad(phi)
            )
            * dx
            - grad(p)[0] * phi * dx
        )
        ...
        #    self.ind_obs : index = 1 if obs available, 0 otherwise, array type
        L = 2.0 * (Hb - self.Hobs) * self.ind_obs * phi * dx
    ...
    def Gradient_Cost(self, b, Hb, Pb, flag_val=False):
        ...
        if not (self.alpha == 0.0):
            ...
            if self.jreg_type == "grad":  # regularization term = "grad"
                ...
                L += (
                    self.alpha
                    * 2.0
                    * inner((grad(b)[0] - mean_slope), grad(phi)[0])
                    * dx
                )

            elif self.jreg_type == "b_b":  # regularization term = "background"
                k_array = FEfct_to_array(self.mesh,project(b - self.b_b, self.V))
                aux_np = npl.solve(self.C_chol.T, k_array)
                aux = array_to_FEfct(self.V,aux_np)
                L += self.alpha*2. * aux * phi * dx
                # L += self.alpha * 2.0 * (b - self.b_b) * phi * dx
        ...
        if (
            flag_val
        ):  # case "gradient test" (validation), scalar prod with 1 is returned
    ...
    def ControlConstraint(self, var, b_1st, Hobs):
        ...
        v_min = b_1st - np.mean(Hobs - b_1st) * 2.0  # MAY BE CHANGED
        v_max = Hobs - 1.0
