TOOLS_DIR=/usr
SIMGRID_DIR=$TOOLS_DIR
VITE_DIR=/usr/bin/vite
...
run() {
        human=${1:-0}
	mkdir -p $out
 	echo $my_mpirun -platform platforms/$cluster_platform -hostfile $hostfile -np $P ${exec:-build/bin/main} -m $m -k $k -n $n -b $b -a $algo -p $p -q $q -i $iter $options
 	$my_mpirun -platform platforms/$cluster_platform -hostfile $hostfile -np $P ${exec:-build/bin/main} -m $m -k $k -n $n -b $b -a $algo -p $p -q $q -i $iter $options &> $out/$algo.out
	...
	echo Found $correct correct GEMM out of $trial
	  ...
	  if [ $human -eq 0 ]; then
	    echo "$cluster_platform,$m,$k,$n,$b,$p,$q,$algo,$la,$gflops"
	  else
	    echo "mxnxk=${m}x${n}x${k},b=$b,p x q = $p x $q | using $algo, (lookahead:$la) => $gflops Gflop/s"
	  fi
	  echo "$cluster_platform,$m,$k,$n,$b,$p,$q,$algo,$la,$gflops" >> $csv
