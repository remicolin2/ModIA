...
int main(int argc, char* argv[]) {
  ...
  if(my_rank == 0){
    ...
    MPI_Send(buffer_send, data_size, MPI_INT, 1, 0, MPI_COMM_WORLD);
    // node 0 receives in its buffer buffer_recv data from node 1
    MPI_Recv(buffer_recv, data_size, MPI_INT, 1, 0, MPI_COMM_WORLD, &status);
    ...
    printf("[%d] sends data to %d\n", my_rank, 0);
    MPI_Send(buffer_send, data_size, MPI_INT, 0, 0, MPI_COMM_WORLD);

    // node 1 receives in its buffer buffer_recv data from node 0
    MPI_Recv(buffer_recv, data_size, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    printf("[%d] received data from %d\n", my_rank, 0);
    printf("MPI process %d received value %d from MPI process %d.\n", my_rank, buffer_recv[0], 0);
  }
  // Si message petit mode asyncrhone car le message peut rentrer dans le buffer sinon c'est mode synchrone
  // A partir de 100000 on bloque.
  free(buffer_send);
  free(buffer_recv);
