...
def pre_train_full(net,b_size,h_dim,x_dim,
    ...
    print('Empirical mean of x0:', torch.mean(x0, dim=0))
    ...
    print(f"Initial loss: {-torch.mean(net.c.forward(ha0).log_prob(x0))}")
...
def train_full(net, b_size, h_dim, x_dim,
    ...
    # Save net weights 
    torch.save(net, "net_weights.pth")
    ### TODO 2.4  
    ha = get_ha0(b_size, h_dim)
    mu = []
    for t in range(T):
        _, ha = net(ha, xt[t+1], yt[t+1])
